...
function! xolox#lua#misc#buffer#is_empty() " {{{1
...
function! xolox#lua#misc#buffer#prepare(...) " {{{1
  ...
  for bufnr in tabpagebuflist()
    if xolox#lua#misc#path#equals(options['path'], bufname(bufnr))
  ...
  if !(found || xolox#lua#misc#buffer#is_empty())
    vsplit
  endif
  ...
  call xolox#lua#misc#buffer#unlock()
  silent %delete
endfunction

function! xolox#lua#misc#buffer#lock() " {{{1
  " Lock a special buffer so that its contents can no longer be edited.
  setlocal readonly nomodifiable nomodified
endfunction

function! xolox#lua#misc#buffer#unlock() " {{{1
