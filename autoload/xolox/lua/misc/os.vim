...
let g:xolox#lua#misc#os#version = '0.5'

function! xolox#lua#misc#os#is_win() " {{{1
  " Returns 1 (true) when on Microsoft Windows, 0 (false) otherwise.
  return has('win16') || has('win32') || has('win64')
endfunction

function! xolox#lua#misc#os#find_vim() " {{{1
  ...
  if has('macunix')
    ...
    call xolox#lua#misc#msg#debug("os.vim %s: Trying MacVim workaround to find Vim executable ..", g:xolox#lua#misc#os#version)
    let segments = xolox#lua#misc#path#split($VIMRUNTIME)
    if segments[-3:] == ['Resources', 'vim', 'runtime']
      let progname = xolox#lua#misc#path#join(segments[0:-4] + ['MacOS', 'Vim'])
      call xolox#lua#misc#msg#debug("os.vim %s: The MacVim workaround resulted in the Vim executable %s.", g:xolox#lua#misc#os#version, string(progname))
    endif
  endif
  if empty(progname)
    call xolox#lua#misc#msg#debug("os.vim %s: Looking for Vim executable named %s on search path ..", g:xolox#lua#misc#os#version, string(v:progname))
    let candidates = xolox#lua#misc#path#which(v:progname)
    if !empty(candidates)
      call xolox#lua#misc#msg#debug("os.vim %s: Found %i candidate(s) on search path: %s.", g:xolox#lua#misc#os#version, len(candidates), string(candidates))
      let progname = candidates[0]
    endif
  endif
  call xolox#lua#misc#msg#debug("os.vim %s: Reporting Vim executable %s.", g:xolox#lua#misc#os#version, string(progname))
  return progname
endfunction

function! xolox#lua#misc#os#exec(options) " {{{1
  ...
  try
    ...
    if has_key(a:options, 'stdin')
      ...
      let cmd .= ' < ' . xolox#lua#misc#escape#shell(tempin)
    endif
    ...
    if !async
      ...
      let cmd = printf('(%s) 1>%s 2>%s', cmd, xolox#lua#misc#escape#shell(tempout), xolox#lua#misc#escape#shell(temperr))
    endif
    ...
    try
      if xolox#shell#can_use_dll()
        ...
        call xolox#lua#misc#msg#debug("os.vim %s: Executing external command using compiled DLL: %s", g:xolox#lua#misc#os#version, cmd)
        let exit_code = xolox#shell#execute_with_dll({'command': cmd, 'async': async})
      endif
    catch /^Vim\%((\a\+)\)\=:E117/
      call xolox#lua#misc#msg#debug("os.vim %s: The vim-shell plug-in is not installed, falling back to system() function.", g:xolox#lua#misc#os#version)
    endtry
    ...
    if !exists('exit_code')
      ...
      if async
        if xolox#lua#misc#os#is_win()
          let cmd = 'start /b ' . cmd
        elseif has('unix')
          let cmd = '(' . cmd . ') &'
        else
          call xolox#lua#misc#msg#warn("os.vim %s: I don't know how to run commands asynchronously on your platform! Falling back to synchronous mode.", g:xolox#lua#misc#os#version)
        endif
      endif
      ...
      if has('unix')
        call xolox#lua#misc#msg#debug("os.vim %s: Generated shell expression: %s", g:xolox#lua#misc#os#version, cmd)
        let cmd = printf('sh -c %s', xolox#lua#misc#escape#shell(cmd))
      endif

      " Let the user know what's happening (in case they're interested).
      call xolox#lua#misc#msg#debug("os.vim %s: Executing external command using system() function: %s", g:xolox#lua#misc#os#version, cmd)
      call system(cmd)
    ...
    if !async
      ...
      if get(a:options, 'check', 1) && exit_code != 0
        ...
        let msg = printf("os.vim %s: External command failed with exit code %d!", g:xolox#lua#misc#os#version, result['exit_code'])
