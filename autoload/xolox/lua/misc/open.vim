...
function! xolox#lua#misc#open#file(path, ...) " {{{1
  ...
  "
  "     :call xolox#lua#misc#open#file('/path/to/my/file', 'firefox', 'google-chrome')
  "
  ...
  if xolox#lua#misc#os#is_win()
    try
      ...
      silent execute printf(command, xolox#lua#misc#escape#shell(a:path))
    endtry
    return
  elseif has('macunix')
    ...
    for handler in s:handlers + a:000
      if executable(handler)
        call xolox#lua#misc#msg#debug("open.vim %s: Using '%s' to open '%s'.", s:version, handler, a:path)
  ...
  throw printf(s:enoimpl, s:script, 'xolox#lua#misc#open#file')
endfunction

function! xolox#lua#misc#open#url(url) " {{{1
  ...
  call xolox#lua#misc#open#file(url, 'firefox', 'google-chrome')
endfunction

function! s:handle_error(cmd, output) " {{{1
  if v:shell_error
    let message = "open.vim %s: Failed to execute program! (command line: %s%s)"
    let output = strtrans(xolox#lua#misc#str#trim(a:output))
    if output != ''
