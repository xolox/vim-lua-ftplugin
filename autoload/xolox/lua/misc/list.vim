...
function! xolox#lua#misc#list#unique(list) " {{{1
...
function! xolox#lua#misc#list#binsert(list, value, ...) " {{{1
...
if 0
  " Tests for xolox#lua#misc#list#binsert().
  let s:list = ['a', 'B', 'e']
  function! s:test(value, expected)
    call xolox#lua#misc#list#binsert(s:list, a:value, 1)
    if s:list != a:expected
      call xolox#lua#misc#msg#warn("list.vim: Test failed! Expected %s, got %s",
