...
function! xolox#lua#misc#timer#stop(...) " {{{1
  ...
  " `xolox#lua#misc#timer#start()` as an argument.
  "
  " [verbose]: http://vimdoc.sourceforge.net/htmldoc/options.html#'verbose'
  " [printf]: http://vimdoc.sourceforge.net/htmldoc/eval.html#printf()
  if (g:timer_enabled || &verbose >= g:timer_verbosity)
    call call('xolox#lua#misc#msg#info', map(copy(a:000), 's:convert_value(v:val)'))
  endif
endfunction

function! xolox#lua#misc#timer#force(...) " {{{1
  ...
  " you pass the list returned by `xolox#lua#misc#timer#start()` as an argument.
  call call('xolox#lua#misc#msg#info', map(copy(a:000), 's:convert_value(v:val)'))
endfunction
...
function! s:convert_value(value) " {{{1
  ...
  elseif !empty(a:value)
    if s:has_reltime
      let ts = xolox#lua#misc#str#trim(reltimestr(reltime(a:value)))
    else
      let ts = localtime() - a:value[0]
    endif
    return xolox#lua#misc#timer#format_timespan(ts)
  else
    return '?'
  endif
endfunction
...
function! xolox#lua#misc#timer#format_timespan(ts) " {{{1
