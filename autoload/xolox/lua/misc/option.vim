...
function! xolox#lua#misc#option#get(name, ...) " {{{1
...
function! xolox#lua#misc#option#split(value) " {{{1
  " Given a multi-value Vim option like ['runtimepath'] [rtp] this returns a
  " list of strings. For example:
  "
  "     :echo xolox#lua#misc#option#split(&runtimepath)
...
function! xolox#lua#misc#option#join(values) " {{{1
  " Given a list of strings like the ones returned by
  " `xolox#lua#misc#option#split()`, this joins the strings together into a
...
function! xolox#lua#misc#option#split_tags(value) " {{{1
  " Customized version of `xolox#lua#misc#option#split()` with specialized
...
function! xolox#lua#misc#option#join_tags(values) " {{{1
  " Customized version of `xolox#lua#misc#option#join()` with specialized
  " handling for Vim's ['tags' option] [tags].
  let values = copy(a:values)
...
function! xolox#lua#misc#option#eval_tags(value, ...) " {{{1
  ...
  for pattern in xolox#lua#misc#option#split_tags(a:value)
    " Make buffer relative pathnames absolute.
    if pattern =~ '^\./'
      let directory = xolox#lua#misc#escape#substitute(expand('%:p:h'))
      let pattern = substitute(pattern, '^.\ze/', directory, '')
    endif
    " Make working directory relative pathnames absolute.
    if xolox#lua#misc#path#is_relative(pattern)
      let pattern = xolox#lua#misc#path#merge(getcwd(), pattern)
    endif
