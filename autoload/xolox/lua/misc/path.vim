...
let s:windows_compatible = xolox#lua#misc#os#is_win()
let s:mac_os_x_compatible = has('macunix')

function! xolox#lua#misc#path#which(...) " {{{1
  ...
  " example:
  "
  "     :echo xolox#lua#misc#path#which('gvim', 'vim')
  "     ['/usr/local/bin/gvim',
  "      '/usr/bin/gvim',
  "      '/usr/local/bin/vim',
  "      '/usr/bin/vim']
  ...
  for program in a:000
    for extension in extensions
      for directory in split($PATH, s:windows_compatible ? ';' : ':')
        let directory = xolox#lua#misc#path#absolute(directory)
        if isdirectory(directory)
          let path = xolox#lua#misc#path#merge(directory, program . extension)
          if executable(path)
...
function! xolox#lua#misc#path#split(path) " {{{1
  ...
  " use the `xolox#lua#misc#path#join()` function.
  if type(a:path) == type('')
    if s:windows_compatible
      if a:path =~ '^[\/][\/]'
        " UNC pathname.
...
function! xolox#lua#misc#path#join(parts) " {{{1
  ...
  " `xolox#lua#misc#path#split()` function and it expects a list of pathname
  " components as returned by `xolox#lua#misc#path#split()`.
  if type(a:parts) == type([])
    if s:windows_compatible
      return join(a:parts, xolox#lua#misc#path#directory_separator())
    elseif a:parts[0] == '/'
...
function! xolox#lua#misc#path#directory_separator() " {{{1
  " Find the preferred directory separator for the platform and settings.
  return exists('+shellslash') && &shellslash ? '/' : '\'
endfunction

function! xolox#lua#misc#path#absolute(path) " {{{1
  ...
  if type(a:path) == type('')
    ...
    elseif xolox#lua#misc#path#is_relative(path)
    ...
    let parts = xolox#lua#misc#path#split(path)
    if s:windows_compatible && parts[0] =~ '^[\/][\/]'
      ...
      let parts[0] = repeat(xolox#lua#misc#path#directory_separator(), 2) . parts[0][2:]
    endif
    return xolox#lua#misc#path#join(parts)
  endif
  return ''
endfunction

function! xolox#lua#misc#path#relative(path, base) " {{{1
  " Make an absolute pathname (the first argument) relative to a directory
  " (the second argument).
  let path = xolox#lua#misc#path#split(a:path)
  let base = xolox#lua#misc#path#split(a:base)
  while path != [] && base != [] && path[0] == base[0]
    call remove(path, 0)
    call remove(base, 0)
  endwhile
  let distance = repeat(['..'], len(base))
  return xolox#lua#misc#path#join(distance + path)
endfunction


function! xolox#lua#misc#path#merge(parent, child, ...) " {{{1
  " Join a directory pathname and filename into a single pathname.
  if type(a:parent) == type('') && type(a:child) == type('')
    " TODO Use xolox#lua#misc#path#is_relative()?
    if s:windows_compatible
...
function! xolox#lua#misc#path#commonprefix(paths) " {{{1
  " Find the common prefix of path components in a list of pathnames.
  let common = xolox#lua#misc#path#split(a:paths[0])
  for path in a:paths
    let index = 0
    for segment in xolox#lua#misc#path#split(path)
      if len(common) <= index
        break
  ...
  return xolox#lua#misc#path#join(common)
endfunction

function! xolox#lua#misc#path#encode(path) " {{{1
...
function! xolox#lua#misc#path#decode(encoded_path) " {{{1
  " Decode a pathname previously encoded with `xolox#lua#misc#path#encode()`.
  return substitute(a:encoded_path, '%\(\x\x\?\)', '\=nr2char("0x" . submatch(1))', 'g')
endfunction

" xolox#lua#misc#path#equals(a, b) - Check whether two pathnames point to the same file. {{{1

if s:windows_compatible
  function! xolox#lua#misc#path#equals(a, b)
    return a:a ==? a:b || xolox#lua#misc#path#absolute(a:a) ==? xolox#lua#misc#path#absolute(a:b)
  endfunction
else
  function! xolox#lua#misc#path#equals(a, b)
    return a:a ==# a:b || xolox#lua#misc#path#absolute(a:a) ==# xolox#lua#misc#path#absolute(a:b)
...
function! xolox#lua#misc#path#is_relative(path) " {{{1
...
function! xolox#lua#misc#path#tempdir() " {{{1
  ...
  if !exists('template')
    throw "xolox#lua#misc#path#tempdir() hasn't been implemented on your platform!"
  endif
  while 1
